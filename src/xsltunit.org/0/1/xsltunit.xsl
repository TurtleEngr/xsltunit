...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://icl.com/saxon" xmlns:xsltu="http://xsltunit.org/0/" exclude-result-prefixes="saxon">
  <xsl:template name="xsltu:assertEqual">
    ...
    <xsl:variable name="result">
      <xsl:call-template name="xsltu:diff">
        <xsl:with-param name="nodes1" select="saxon:nodeSet($nodes1)"/>
        <xsl:with-param name="nodes2" select="saxon:nodeSet($nodes2)"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:call-template name="xsltu:assert">
      <xsl:with-param name="id" select="$id"/>
      <xsl:with-param name="test" select="not(saxon:nodeSet($result)//xsltu:no-match)"/>
      <xsl:with-param name="message" select="saxon:nodeSet($result)"/>
    </xsl:call-template>
  </xsl:template>
  <xsl:template name="xsltu:assertNotEqual">
    ...
    <xsl:call-template name="xsltu:assert">
      <xsl:with-param name="id" select="$id"/>
      <xsl:with-param name="test" select="saxon:nodeSet($result)//xsltu:no-match"/>
